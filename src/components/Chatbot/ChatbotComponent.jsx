...
import {getEmbeddings, cosineSimilarity } from "../../utils/aiServices"; // Path adjusted
...
const ChatbotComponent = () => {
    ...
    const [preparedKnowledgeBase, setPreparedKnowledgeBase] = useState(KNOWLEDGE_BASE_DATA);
 ...
 // Preparing the knowledge base data
   useState(() => {
    ...
    useEffect(() => {
        if(chatMessagesRef.current) {
            chatMessagesRef.current.scrollTo({
              // Scrolls the chat container to a specific position
    ...
    const getAIResponse = async (userMessageText) => {
       ...
       if (!isOnline) {
         setLoading(false);
         return "It seems you're offline. Please check your internet connections.";
        }
        ...
        try {
            ...
            for(const intent in preparedKnowledgeBase) {
                ...
                for (const exampleEmbeddings of intentEmbeddings) {
                    const score = cosineSimilarity(userEmbedding, exampleEmbeddings);
    ...
    const handleSendMessage = async () => {
      ...
      setMessages((prevMassages) => [...prevMassages, newUserMessage]);
      setInputValue("");
      if (!isOnline) {
       ...
       setMessages((prevMassages) => [...prevMassages, botResponse]);
    };
                    ...
                    <>
                        ...
                        {!isOnline && (
                            <p className="chatbot-message offline-message">
                               :no_entry_sign:: You are currently offline. Please check your internet connection.
                            </p>
                        )}
                        <div className="chatbot-message" ref={chatMessagesRef}>
                            {messages.map((message, index) => (
                                <div key={index} className={`message-row ${message.sender}`}>
                                    {message.sender === "bot" && (
                                        <div className="avator bot-avator">:robot_face:</div>
                                    )}
                                    <p className={`${message.sender}-message`}>{message.text}</p>
                                    {message.sender === "user" && (
                                       <div className="avator user-avator">:bust_in_silhouette:</div>
                                    )}
                                </div>
                            ))}
                            {loading && (
